...
local function ApplyClothes()
	...
	if DoesEntityExist(playerPed) then
		Citizen.CreateThread(function()
			...
			if gender == 0 then
				TriggerEvent('qb-clothing:client:loadOutfit_Karma_Adapted', Config.Uniforms.male)
			else
				TriggerEvent('qb-clothing:client:loadOutfit_Karma_Adapted', Config.Uniforms.female)
			end
		end)
	end
end
